...
async def crawl_news_items(
    ...
    # Load data
    data = None
    with open(news_search.results_filepath, "r", encoding="utf-8") as f1:
        data = json.load(f1)
        print(f"Number of URLs to parse: {len(data)}")
        # print(json.dumps(data[0], indent=2))

    parsed_urls = []
    with open(news_search.parsed_urls_filepath, "r", encoding="utf-8") as f2:
        parsed_urls = json.load(f2)
        print(f"\nNumber of parsed URLs: {len(parsed_urls)}")
        # print(parsed_urls)

    failed_urls = []
    with open(news_search.failed_urls_filepath, "r", encoding="utf-8") as f3:
        ...
        for item in data:
            ...
            if not news_items:
                ...
                for news_item in news_items:
                    dut.save_single_news_item_to_csv(
                        news_item, news_search.results_dirname, "parsed_news_items.csv"
                    )
            break
            article_number += 1  # Move to the next article
    ...
    dut.save_to_json(
        ...
        filepath=news_search.failed_urls_filepath,
    )
    ...
    #         all_news_items, "all_parsed_news_items.csv", news_search.results_dirname
    #     )
    #     dut.save_to_json(
    #         results=list(set(parsed_urls)),
    #         filepath=news_search.parsed_urls_filepath,
    #     )
    # else:
...
async def main():
    ...
    # parsed_urls_filepath = os.path.join(news_search.results_dirname, "parsed_urls.json")
    ...
    await crawl_news_items(news_search)  # , source_to_parse="ThePrint")
